...
2) Write items such that each item reflects the construct definition
...
Example file: [download here](items_group20.csv)

To ensure full marks for your work with items be sure you do all of the following - exactly as noted - included upper/lower case distinctions. The smallest deviation - even in the case of the letters will result in a loss or marks. Why are we so particular with submissions? Once the items are submitted they are processed by the computer and the smallest deviation will cause the computer to crash.
...
* **item_name** column. Use a meaningful and moderate length name for each item.
  ...
  * Be sure to indicate in the item name the number of Likert points (e.g., likert7 for 7 points).
  * Be sure to indicate if an item is a reverse-keyed item by included "rev" after the likert indication (e.g., likert7rev). 
...
* **item_text** column. 
  ...
  * Begin each item text with a capital letter
...
* **type** column. 
  ...
  * Simply copy/paste the content of this column so that it is present for all items.
