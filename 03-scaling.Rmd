...
We can obtain the mean for the cm and inches columns with the command below:
...
We can obtain the standard deviation (population formula) for the cm and inches columns with the command below:
...
An alternative approach to representing scores is using *z*-scores. An advantage of *z*-scores is that when scores are expressed in this manner the mean of a column is 0 and the standard deviation (population formula) is 1.0. The advantage of using this approach is that by simply looking a someone's *z*-score we can tell if they are above or below the average for that group (i.e., the group the mean and standard deviation were obtained from). Specifically, when *z*-scores are positive this indicates that they are above the mean; likewise, when *z*-score are negative this indicates that they are below the mean. The value of the *z*-score indicates how far a person is from the mean in standard deviation units. 

In our data John's height can be expressed as a *z*-score of 2.08408360. Because this is a positive value we know John is taller than the average person. Likewise, the value of 2.08408360 indicates that John is 2.08408360 standard deviations taller than the average person. This means that John is quite tall.


#### Hand calculation

We can convert an entire column of numbers to *z*-scores with a simple calculation. However, before we begin, we need to know the mean and standard deviation (population formula) for the original column.

Recall the heights in centimeters:
...
In general we use the formula below to convert a person's score in the original units to their *z*-score:
...
Here is an example calculation for John:
...
We can do the same in R with the code below:
...
We can obtain the standard deviation (population formula) for the cm, inches, and *z*-score columns with the command below:
...
We can obtain the standard deviation (population formula) for the cm, inches, *z*-score, and T-score columns with the command below:
...
In the above data, the z_score column was calculated using the mean and standard deviation for all participants.

Specifically, all participants in this data set the mean and standard deviation are:
...
These mean and standard deviation values were used to calculate John's z-score (and everyone elses z-score too):
