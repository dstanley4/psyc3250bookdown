...
To obtain the mean and standard deviation (population formula) for each column in the data set we use the commands below. These are explained in the previous "Handling data with the tidyverse" chapter. You can see in the code below that by specifying .cols as "where(is.numeric)" that we obtain descriptive statistics for any numeric column.
...
We can obtain the mean and standard deviation (population formula) for the cm and inches columns with the command below. Note that we added "%>% t()" in this version which reads as "and then transpose". This extra command simply transposes the row to a column so the results don't go off the right side of your screen.
...
The advantage of *z*-scores is that by simply looking a person's *z*-score we can tell if they are above or below the mean for the frame of reference. Specifically, when a *z*-score is positive this indicates that a person is above the mean of the frame of reference; likewise, when a *z*-score are negative this indicates that the person is below the mean of the frame of reference. The magnitude of the *z*-score indicates how distant that person is from the mean of the frame of reference in standard deviation units. 

In we examine our data we find that John has a height of 190 cm. We use the heights of the people in our data set as a frame of reference. Moving forward in our notation this will be the "data" frame of reference. Using all of the people in our data set we calculate a mean height of 173.3 cm and a standard deviation (population formula) of 8.013114 cm. We use these values as a frame of reference when calculating John's *z*-score. 
...
We can use R to calculate the *z*-score for everyone in our data set. Because we are using the data set itself as the frame of reference we can use to code to represent the mean and standard deviation for the data set - our frame of reference. Previously, we found:
...
Therefore, we write the code below to create *z*-scores with the entire data set as the frame of reference. More specifically, we are creating a new column in the data set called z_data (to indicate the frame of reference).
...
We could have used the code below which has the computer calculate the mean and standard deviation as part of the command. However, this code is harder to follow - so we opted to use the code above.
...
We use the same code as before to display the data set. Notice that we have added the arrange(z_data) command to sort the data by z-scores.
...
Notice that each person now has a *z*-score. Just by looking at the positive/negative sign for the *z*-score we can tell if a person is taller/shorter than the mean for the data set (our frame of reference). This is even more important if the data is not sorted. Indeed, we can tell just by looking a single person's score if they are above or below the mean of the frame of reference using *z*-scores.
