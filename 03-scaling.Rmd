...
Both inches and centimeters are ratio-scale scale heights. The zero point on the measurement scale is meaningful and not arbitrary. There are bennefits to this approach it's the zero point is defined independent of the data. Below we examine two approaches to representing heights (z-scores and T-scores) for which the zero-point depends on the nature of the data. This initially seems like an odd approach - but you will that there are many bennefits to expressing heights in this manner. 



### z-scores

An alternative way to reprsenting scores is using z-scores. An advantage of z-scores is that when scores are expressed in this manner the mean of a column is 0 and the standard deviation (population formula) is 1.0. The advtange of using this approach is that by simplying looking a someone's z-score we can tell if they are above or below the average for that group (i.e., the group the mean and standard deviation were obtained from). Specifically, when z-scores are positive this indicates that they are above the mean; likewise, when z-score are negative this indicates that they are below the mean. The value of the z-score indicates how far a person is from the mean in standard deviation units. 
