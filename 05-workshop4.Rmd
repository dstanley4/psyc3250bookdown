...
When we discuss Cronbach's alpha you know we are dealing with the original form of the data. That is, we are dealing with the actual scores there reported by particpants - rather than some transformed version of those scores.
...
We have a new tibble now, called **items**, that has just the affective commitment item data in it. You can see that tibble via the glimpse() commmand:
...
Finally, we use the covariance matrix in the Cronbach's alpha formula below.
...
This the value of Cronbach's alpha is .7727002, or .77. The affect_sum column has variance of 3.06. This is an indication of the differences among people with respect to **observed** scores (i.e., the actual values in affect_sum column). Of course, not all of the variability in observed scores is due to differences among people (i.e., true scores). Some of the variability in observed scores (in the affect_sum column) is due to random measurement error. A Cronbach's alpha of .77 indicates that we **estimate** 77% of the variabilty in scores in the affect_sum column is due to actual differences among people (i.e., variability in true scores).

### Alpha via psych package

You can obtain Cronbach's alpha using the alpha comamnd from the **psych** package. However, because we didn't use library(psych), because it breaks the tidyverse, we add psych:: in front of the alpha() command.
...
As before we use the skim() command to get the standard deviation of each item. Examine the numeric.sd column in the output. You see a value of 1.0 for each item. This indicates that is were to add the values in these columns together to create a new column - each column would contribute equally to the creation of that new column. Using our cooking analolgy, we now using the same amount of each ingredient.
...
We can see the variance of this column is 25.0. That is, the variance of observed scores on affective commitment scale is 25.0. This variability in observed scores is composed of 1) actual variability in commitment levels across participants (i.e., true score variance) and 2) measurement error.

We might wonder, of the observed variance of 25.0, what proportion is due to actual variability in people's affective commitment levels - and not measurement error. That is, we might wonder, what proportion of the 3.06 variance is due to variability in true scores. We determine that proportion by calculating Standardized alpha.
...
Next, we calculate the correlation matrix based on those items.
...
Finally, we use the correlation matrix in the usual alpha formula below.
...
This the value of Standardized alpha is .xxx, or .xx. The z_affect_sum column has variance of 25.0 This is an indication of the differences among people with respect to **observed** scores (i.e., the actual values in affect_sum column). Of course, not all of the variability in observed scores is due to differences among people (i.e., true scores). Some of the variability in observed scores (in the z_affect_sum column) is due to random measurement error. A Standardized alpha of .77 indicates that we **estimate** 77% of the variabilty in scores in the affect_sum column is due to actual differences among people (i.e., variability in true scores).

Note that in these data, the Standardized alpha value is very close to the Cronbach's alpha value. This will not always be the case. In many circumstances, the two values will differ considerably.

### Standardized Alpha via psych package

You can obtain standardized alpha using the alpha comamnd from the **psych** package. However, because we didn't use library(psych), because it breaks the tidyverse, we add psych:: in front of the alpha() command.
